...
export function toTreatmentDto(record: TreatmentRecord, now: Date = new Date()): TreatmentDto {
  return {
    ...
    completionDate: toCalendarDate(record.completionDate ?? null),
    debondPerformed: record.debondPerformed ?? null,
    debondDate: toCalendarDate(record.debondDate ?? null),
    retentionRequired: record.retentionRequired ?? null,
    finalMediaIds: (record.finalMediaIds ?? []).map((id) => id.toString()),
